...
| 4 | **Kill all VS Code terminals** | Use the VS Code command `workbench.action.terminal.killAll` (via the `run_vscode_command` tool) to kill all open terminals, including hidden ones from previous study participants. This is more reliable than grep-based approaches since it targets terminals managed by the editor directly. Skip gracefully if the command is unavailable. |
